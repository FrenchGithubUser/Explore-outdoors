...
function initLocation() {
	watchLocation(
		...
		function () {
			var errorCoordsContainer = document.getElementById(
				"errorCoordsContainer"
			); //container to print the coordinates or an error message
			errorCoordsContainer.innerHTML = '<div style="margin-top:20px;"></div>';
			errorCoordsContainer.insertAdjacentHTML(
				"beforeend",
				"<b>Please enable your location services and reload the page.</b>"
			);
		}
	);
}
...
var icon = L.icon({
	iconUrl: "../../common_files/position.svg",
	iconSize: [25, 25],
...
function addSpotsToMap(spots) {
	for (spot in spots) {
		...
		marker.bindPopup(
			"<p><a href='javascript:displaySpotInfo(\"" +
			spotName +
			'","' +
			lat +
			'","' +
...
var spotDescriptionContainer = document.getElementById(
	"spotDescriptionContainer"
);
var spotMetadataContainer = document.getElementById("spotMetadataContainer");
var spotCoordinatesContainer = document.getElementById(
	"spotCoordinatesContainer"
);
...
function displaySpotInfo(name, lat, long) {
	...
	spotNameContainer.insertAdjacentHTML(
		"beforeend",
	...
	spotInfoRequest.onload = function () {
		...
		for (i = 0; i < spotInfo[0].p; i++) {
			picCounter += 1;
			if (picCounter > 2) {
				spotPicturesContainer.insertAdjacentHTML(
					"beforeend",
					"<div>" + spotPictures + "</div>"
				);
				spotPicturesContainer.insertAdjacentHTML(
					"beforeend",
					'<div style="margin-top:3%;"></div>'
		...
		spotDescriptionContainer.insertAdjacentHTML(
			"beforeend",
			"<p>" + spotInfo[0].d + "</p>"
		);
		spotDescriptionContainer.insertAdjacentHTML(
		...
		//adding the heights
		spotMetadataContainer.insertAdjacentHTML(
			"beforeend",
			"<img class='logo' src='spot_height.svg' width=5%> Heights from " +
			spotInfo[0].mi +
			"m to " +
			spotInfo[0].ma +
			"m<span style='margin-right:3%;'></span>"
		);
		...
		*/
		...
		if (spotInfo[0]["v"].length > 0) {
			spotVideosContainer.insertAdjacentHTML(
				"beforeend",
				'<div style="margin-top:3%;"></div>'
			);
			spotVideosContainer.insertAdjacentHTML(
				"beforeend",
				"<img class='logo' src='video.svg' width=4%><span style='margin-right:2%;'></span>Videos at the spot : "
			);
	...
	spotCoordinatesContainer.insertAdjacentHTML(
		"beforeend",
		'<div style="margin-top:3%;"></div>'
	);
	spotCoordinatesContainer.insertAdjacentHTML(
		"beforeend",
		"<img class='logo' src='coordinates.svg' width='4%'> Coordinates : " +
		lat +
		", " +
		long
	);
...
ourRequest.open(
	"GET",
	"https://Explore-outdoors.org/cliff_diving/cliff_diving_home/spotCoordinates.json"
);
